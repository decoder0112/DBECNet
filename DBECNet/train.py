...
def train(args, train_loader, model, optimizer, epoch, max_batches, cur_iter=0, lr_factor=1.):
    ...
    for iter, batched_inputs in enumerate(train_loader):
        ...
        if iter % 5 == 0:
            logging.info(
                'iteration: [%d/%d]  precision: %.4f  recall: %.4f  miou: %.4f  F1: %.4f  lr: %.7f  loss: %.4f  time:%.3f h' % (
                    iter + cur_iter, max_batches * args.max_epochs, precision, recall, miou, f1, lr, loss.data.item(),
                    res_time))
...
def trainValidateSegmentation(args):
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        logging.info("Epoch %d: val_loss: %.4f oa: %.4f  precision: %.4f  recall: %.4f  miou: %.4f  F1: %.4f" \
                     % (epoch + 1, lossVal, score_val['OA'], score_val['precision'], score_val['recall'], score_val['Iou'],
                        score_val['F1']))
        torch.cuda.empty_cache()
        # 绘制损失曲线
        plt.plot(lossTr, label='Train Loss')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--file_root', default="LEVIR-cd256", help='Data directory | LEVIR | WHU | SYSU')
    parser.add_argument('--inWidth', type=int, default=256, help='Width of RGB image')
    parser.add_argument('--inHeight', type=int, default=256, help='Height of RGB image')
    parser.add_argument('--max_steps', type=int, default=68200, help='Max. number of iterations')  #####
    parser.add_argument('--num_workers', type=int, default=4, help='No. of parallel threads')  # 4
    parser.add_argument('--batch_size', type=int, default=32, help='Batch size')  # 32   16
    ...
    parser.add_argument('--savedir', default='./LEVIR.log', help='Directory to save the results')
    parser.add_argument('--resume', default=None, help='Use this checkpoint to continue training | '
                                                       './results_ep100/checkpoint.pth.tar')
    parser.add_argument('--log_dir', default='./LEVIR.log', help='File that stores the training and validation logs')
